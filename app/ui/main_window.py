...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        self.before_check.setVisible(False)
        self.before_check.setEnabled(False)
        ...
        self.after_check.setVisible(False)
        self.after_check.setEnabled(False)
        ...
        self.date_filter_master = QCheckBox("Limit by date")
        date_layout.addWidget(self.date_filter_master)

        self.date_filter_panel = QWidget()
        date_filter_panel_layout = QGridLayout(self.date_filter_panel)
        date_filter_panel_layout.setContentsMargins(20, 0, 0, 0)
        date_filter_panel_layout.setHorizontalSpacing(8)
        date_filter_panel_layout.setVerticalSpacing(6)
        date_filter_panel_layout.addWidget(QLabel("From:"), 0, 0)
        date_filter_panel_layout.addWidget(self.after_date, 0, 1)
        date_filter_panel_layout.addWidget(self.after_time, 0, 2)
        date_filter_panel_layout.addWidget(QLabel("To:"), 1, 0)
        date_filter_panel_layout.addWidget(self.before_date, 1, 1)
        date_filter_panel_layout.addWidget(self.before_time, 1, 2)
        date_filter_panel_layout.setColumnStretch(1, 1)
        date_layout.addWidget(self.date_filter_panel)
        ...
        self.date_filter_master.toggled.connect(self.update_filter_controls)

        options_group = QGroupBox("Output Format")
        ...
        self.txt_format_section = QWidget()
        txt_format_section_layout = QVBoxLayout(self.txt_format_section)
        txt_format_section_layout.setContentsMargins(0, 4, 0, 0)
        txt_format_section_layout.setSpacing(6)

        txt_format_header_layout = QHBoxLayout()
        txt_format_header_layout.setContentsMargins(0, 0, 0, 0)
        txt_format_header_layout.setSpacing(6)
        txt_format_header_layout.addWidget(QLabel("TXT Formatting"))
        txt_format_header_layout.addStretch(1)
        self.txt_format_advanced_button = QToolButton()
        self.txt_format_advanced_button.setCheckable(True)
        self.txt_format_advanced_button.setChecked(False)
        self.txt_format_advanced_button.setAutoRaise(True)
        txt_format_header_layout.addWidget(self.txt_format_advanced_button)
        txt_format_section_layout.addLayout(txt_format_header_layout)

        self.txt_format_summary_label = QLabel("Includes: edited, pins, replies")
        txt_format_section_layout.addWidget(self.txt_format_summary_label)

        self.txt_format_details_panel = QWidget()
        txt_format_details_layout = QVBoxLayout(self.txt_format_details_panel)
        txt_format_details_layout.setContentsMargins(20, 0, 0, 0)
        txt_format_details_layout.setSpacing(4)
        txt_format_details_layout.addWidget(self.include_edits)
        txt_format_details_layout.addWidget(self.include_pins)
        txt_format_details_layout.addWidget(self.include_replies)
        txt_format_section_layout.addWidget(self.txt_format_details_panel)
        options_layout.addWidget(self.txt_format_section)

        self.export_txt.toggled.connect(self._update_txt_format_controls)
        self.txt_format_advanced_button.toggled.connect(self._update_txt_format_controls)
        ...
        output_layout.addWidget(self.open_folder_toggle)
        ...
        execute_group = QGroupBox("Execute")
        execute_layout = QVBoxLayout(execute_group)
        execute_layout.setSpacing(6)
        execute_layout.addLayout(actions_row)
        execute_layout.addWidget(self.batch_progress_label)
        execute_layout.addWidget(self.progress)
        execute_layout.addWidget(self.batch_progress)
        ...
        execute_group.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Maximum)
    ...
    def update_filter_controls(self) -> None:
        enabled = self.date_filter_master.isChecked()
        self.before_check.setChecked(enabled)
        self.after_check.setChecked(enabled)
        self.date_filter_panel.setVisible(enabled)
        self.before_date.setEnabled(enabled)
        self.before_time.setEnabled(enabled)
        self.after_date.setEnabled(enabled)
        self.after_time.setEnabled(enabled)

    def _update_txt_format_controls(self) -> None:
        txt_enabled = self.export_txt.isChecked()
        expanded = txt_enabled and self.txt_format_advanced_button.isChecked()

        self.txt_format_section.setVisible(txt_enabled)
        self.txt_format_details_panel.setVisible(expanded)
        self.txt_format_summary_label.setVisible(txt_enabled and not expanded)
        self.txt_format_advanced_button.setText(
            self.tr("Advanced {arrow}").format(arrow=("▾" if expanded else "▸"))
        )
        self.include_edits.setEnabled(txt_enabled)
        self.include_pins.setEnabled(txt_enabled)
        self.include_replies.setEnabled(txt_enabled)
